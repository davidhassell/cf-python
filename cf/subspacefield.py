...
class SubspaceField(mixin.Subspace):
    ...
    **Examples:**
    ...
    def __call__(self, *args, **kwargs):
        ...
        Creation of a new field construct which spans a subspace of
        the domain of an existing field construct is achieved by
        identifying indices based on the metadata constructs.
    
        The subspacing operation also subspaces any metadata
        constructs of the field construct (e.g. coordinate metadata
        constructs) which span any of the domain axis constructs that
        are affected. The new field construct is created with the same
        properties as the original field construct.
        ...
        * Conditions may also be applied to multi-dimensional metadata
        ...
        :Parameters:
            ...
            positional arguments: *optional*
                ...
                provides a different type of subspace, plus a testing
                mode:
        ...
        :Returns:
            ...
            `Field` or `bool`
                An independent field construct containing the subspace
                of the original field. If the ``'test'`` positional
                argument has been provided then returns `True` or
                `False` depending on whether or not it is possible to
                create specified subspace.
    
        **Examples:**
    
        There are further worked examples :ref:`in the tutorial
        <Subspacing-by-metadata>`.
