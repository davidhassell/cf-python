...
#        instance._Data = Data
    ...
    def __getitem__(self, indices):
        ...
        * When two or more dimensions’ indices are sequences of
          ...
          Fortran). This is the same indexing behaviouqr as on a
          Variable object of the netCDF4 package.
    
        * For a dimension that is cyclic, a range of indices specified
          by a `slice` that spans the edges of the data (such as
          ``-2:3`` or ``3:-2:-1``) is assumed to "wrap" around, rather
          then producing a null result.
        ...
        **Examples:**
        ...
        ...    True, True, True, True, True]].shape
        (9, 64, 128)
        ...
        if roll:
            axes = data._axes                
            if data._cyclic.intersection([axes[i] for i in roll]):
                raise IndexError(
                    "Can't take a cyclic slice of a non-cyclic axis"
                )
                
            new = self.roll(axis=tuple(roll.keys()),
        ...
        new_data = new_data[tuple(findices)]
    ...
    **Examples:**
    ...
    def indices(self, *mode, **kwargs):
        ...
        **Auxiliary masks**
        ...
        extent of the subspace is defined only by the values of
        elements three and onwards. In this case the second element
        contains an "auxiliary" data mask that is applied to the
        subspace after its initial creation, in order to set
        unselected locations to missing data.
        ...
        **Examples:**
        ...
        if "exact" in mode:
            _DEPRECATION_ERROR_ARG(
                self, "indices", "exact",
                "Keywords to 'indices' are now never interpreted as "
                "regular expressions."
            )  # pragma: no cover

        if len(mode) > 1:
            raise ValueError(
                f"Can't provide more than one positional argument. "
        ...
        if auxiliary_mask:
            ...
            for axes, mask in auxiliary_mask.items():
                # Transpose the mask so that its axes are in the same
                # relative order as the field data axes
                new_axes = [a for a in data_axes if a in axes]
                new_order = [axes.index(a) for a in new_axes]
                mask.transpose(new_order, inplace=True)
 
                # Add size 1 dimensions to the mask to corresposnd to
                # all of the field data dimensions that are currently
                # missing
                shape = mask.shape
                new_shape = []
                i = 0
...
#        # --- End: if
    ...
    **Examples:**
    ...
    >>> f.squeeze(['dim2'], inplace=True)
        ...
        axes, iaxes = self.parse_axes(axes, remove_duplicates=True)
        # TODODASK - push remove_duplicates to Data's parse_axes
    ...
    **Examples:**
    ...
    def transpose(self, axes=None, constructs=False, inplace=False,
        ...
        By default the order of the axes is reversed, but any ordering
        may be specified by selecting the axes of the output in the
        required order.
    
        By default metadata constructs are not transposed, but they
        may be if the *constructs* parameter is set.
    
        .. seealso:: `domain_axis`, `flatten`, `insert_dimension`,
                     `flip`, `squeeze`, `unsqueeze`
        ...
        :Parameters:
            ...
            axes: (sequence of) `str` or `int`, optional
                ...
                provided, or if no axes are specified then the axis
                order is reversed.
    
            constructs: `bool`
                If True then metadata constructs are also transposed
                so that their axes are in the same relative order as
                in the transposed data array of the field. By default
                metadata constructs are not altered.
        ...
        :Returns:
            ...
            `Field` or `None`
                The field construct with transposed data, or `None` if
                the operation was in-place.
        ...
        **Examples:**
        ...
        axes, iaxes = self.parse_axes(axes)
...
#                'subset', *axes).filter_by_axis('and', *axes2))
    ...
    def roll(self, axis, shift, inplace=False, i=False, **kwargs):
        ...
        :Parameters:
            ...
            axis: (sequence of) `int` or `str`
                The axis or axes along which elements are
                shifted. Each axis is defined that which would be
                selected by passing the given axis description to a
                call of the `domain_axis` method. For example, for a
        ...
        :Returns:
            ...
            `Field` or `None`
                The rolled field, or `None` if the operation was
                in-place.
