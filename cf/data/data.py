...
class Data(DataClassDeprecationsMixin, Container, cfdm.Data):
    ...
    **Cyclic axes**
    ...
    def __new__(cls, *args, **kwargs):
        """Store component classes. TODODATA"""
    ...
    def __init__(
        ...
        **Examples**
        ...
        ...             units=cf.Units('m/s'), fill_value=-999)
        >>> d = cf.Data('fly')
        >>> d = cf.Data(tuple('fly'))

        """
        if source is not None:
            super().__init__(source=source, copy=copy, _use_array=_use_array)
        else:
            super().__init__(
                array=array
                units=units,
                ...
                to_memory=to_memory,
                init_options=init_optoins,
                _use_array=_use_array,
            )
            
            # Set whether or not we're sure that the Data instance has
    ...
    @classmethod
    def _concatenate_post_process(cls, data, axis, processed_data):
        """Check and conform the units of the data.
        ...
        :Returns:
            ...
            `{{class}}`
                Returns *data*, possibly modified in-place TODO

        """
        data = super()._concatenate_xxx(data, axis, processed_data)
    ...
    def diff(self, axis=-1, n=1, inplace=False):
        ...
        **Examples**
        ...
        units = self.Units
        if units.isreftime:
            units = self._Units_class(units._units_since_reftime)
    ...
    def masked_where(self, condition, inplace=False):
        ...
        **Examples**
        ...
        """
        d = _inplace_enabled_define_and_cleanup(self)
        d.where(condition, masked, None, inplace=True)
        return d
                ...
                if units1.isreftime:
                    if units0.equivalent(units1):
                        ...
                        if not units0.equals(units1):
                            data1 = data1.copy()
                            data1.Units = units0
                        return data0, data1, self._Units_class(_ut_unit=units0._ut_unit)
                    else:
    ...
    def _cyclic(self):
        ...
        """
        out = self._custom.get("_cyclic", _empty_set)
    ...
    def add_file_location(self, location):
        ...
        `add_file_directory` method instead.
        ...
        **Examples**
        ...
        _DEPRECATION_ERROR_METHOD(
            self,
            'add_file_location',
            message="Use the 'add_file_directory' method instead.",
            version="NEXTVERSION",
            removed_at="TODODATA"
        )
    ...
    @_inplace_enabled(default=False)
    def unique(self, split_every=None, inplace=False):
    ...
    def override_calendar(self, calendar, inplace=False, i=False):
        ...
        **Examples**
        ...
        """
        d = _inplace_enabled_define_and_cleanup(self)
        d._Units = self._Units_class(d.Units._units, calendar)
        return d
