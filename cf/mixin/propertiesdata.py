...
class PropertiesData(Properties):
    ...
    def __array__(self, *dtype):
        ...
        raise ValueError("{self.__class__.__name__} has no data")
    ...
    def __setitem__(self, indices, value):
        ...
        try:
            ...
            raise ValueError(
                f"Can't assign to a {self.__class__.__name__} "
                f"from a {value.__class__.__name__} with no data"
            )
    ...
    def _period(self):
        """TODODASK"""
        return self._custom.get("period")
        ...
        **Examples**
        ...
        if not units.isreftime:
            raise AttributeError(
                f"{self.__class__.__name__} doesn't have reference "
                "date-time units"
            )
    ...
    def reference_datetime(self, value):
        ...
        if not units.isreftime:
            raise AttributeError(
                "Can't set 'reference_datetime' for non reference "
                "date-time units"
            )

        units = units.units.split(" since ")
        try:
            self.units = "{units[0]} since {value}"
        except (ValueError, TypeError):
            raise ValueError(
                "Can't override reference date-time {units[1]!r} "
                "with {value!r}"
            )
    ...
    def units(self):
        ...
        **Examples:**
        ...
        if value is None:
            raise AttributeError(
                f"{self.__class__.__name__} doesn't have CF "
                "property 'units'"
            )
    ...
    def period(self, *value):
        ...
        :Parameters:
            ...
            value: optional
                The period. The absolute value is used.  May be set to
                any numeric scalar object, including `numpy` and
                `Data` objects. The units of the radius are assumed to
                be the same as the data, unless specified by a `Data`
                object.

                If *value* is `None` then any existing period is
                removed from the construct.

        :Returns:

            `Data` or `None`
                The period prior to the change, or the current period
                if no *value* was specified. `None` is always returned
        ...
        if value is not None:
            ...
            if not units:
                ...
                if units.equivalent(self.Units):
                    ...
                    raise ValueError(
                        f"Period units {units!r} are not equivalent "
                        f"to data units {self.Units!r}"
                    )
            # --- End: if
    ...
    def dtype(self):
        ...
        **Examples:**
        ...
        if data is None:
            raise AttributeError(
                "{self.__class__.__name__} doesn't have attribute 'dtype'"
            )
    ...
    def hardmask(self):
        ...
        **Examples:**
        ...
        if data is None:
            raise AttributeError(
                "{self.__class__.__name__} doesn't have attribute 'hardmask'"
            )
    ...
    def concatenate(cls, variables, axis=0, _preserve=True):
        ...
        data = Data.concatenate([v.get_data() for v in variables], axis=axis)
    ...
    def equivalent(self, other, rtol=None, atol=None, traceback=False):
        ...
        *rtol* parameters.
        ...
        if set(self_special) != set(other_special):
            if traceback:
                print(
                    "{self.__class__.__name__}: Different attributes: "
        ...
        for attr, x in self_special.items():
            ...
            if not result:
                if traceback:
                    print(
                        "{self.__class__.__name__}: Different {attr} "
                        "attributes: {x!r}, {y!r}"
                    )
                return False
        # --- End: for
    ...
    def convert_reference_time(
        ...
        **Examples:**
        ...
        if not self.Units.isreftime:
            raise ValueError(
                f"{self.__class__.__name__} must have reference "
                f"time units, not {self.Units!r}"
            )
